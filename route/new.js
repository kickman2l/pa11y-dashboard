...
function route(app) {
    ...
    app.express.post('/new', (request, response) => {
        ...
        if (request.body.actions) {
            parsedActions = request.body.actions
                .split(/[\r\n]+/)
                .map((action) => {
                    return action.trim();
                })
        ...
        const destinationUrl = parseUrl(request.body.url);
        const isAllowed = _.find(domains.allowed, function (url) {
            return compareUrls(url, destinationUrl.resource);
        });

        console.log(isAllowed, destinationUrl, domains);

        if (isAllowed === undefined) {
            return response.render('new', {
                error: 'Not allowed url',
                task: {},
            });
        }
